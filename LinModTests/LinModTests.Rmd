...
Other more complex model includes the `age` and `smoke` variables   

```{r}
# Multipredictor model2
cholBmi_mod2 <- lm(chol ~ bmi + arcus + smoke + age, data = WCGS_n)
...
To compare the two models we use the null hypothesis: $H_0$: both, the full and the nested models fit the data equally well. (if you cannot discard $H_0$ therefore you should take the nested model, Occam's razor). And the alternative hypothesis is $H_1$: the full model significantly outperforms the nested model (conclusion: you keep the full model).  
We can estimate the comparison of a full model and nested models using the `anova` R function, or other more complex methods like the likelihood ration test from `lmtest` package, to compare `cholBmi_mod2` to `cholBmi_mod1` models with anova and the likelihood ratio test we run.  

```{r}
# using the anova R test to compare the models
anova(cholBmi_mod2, cholBmi_mod1)
...
If the p-value associated with the extra parameters is low, it suggests that removing it does significantly affect the model's explanatory power, suggesting that it is better to keep the full model.

## Testing for nonlinearities  

The Test of Concavity/Convexity, often referred to as the Rainbow Test, is a statistical method used primarily in regression analysis to determine whether a model is correctly specified as linear or if it exhibits non-linear characteristics, such as convexity or concavity. The test is constructed assuming the linear model is correct, Null Hypothesis: The null hypothesis of the test posits that the regression model is correctly specified as linear.  
A low p-value (typically below 0.05) would lead to rejection of the null hypothesis, suggesting that the model may not be linear.  
...
We may conclude that the model of cholesterol with age is linear.
