...
$$ Odds = \frac {p}{1−p} $$  
Thus, the log-odds can be expressed mathematically as:  
$$ Log \space Odds = \ln(\frac{p}{1-p}) $$ 
We also have the function  
$$ p = [1 + \exp(-x)]^{-1} = \frac{1}{1+\exp(-x)} $$  
that is called the logistic function, has unlimited range for $x$ while $p$ is restricted to range from $0$ to $1$. 
```{r}
# Logistic function
...
Logistic regression uses log-odds to model probabilities because it allows for predictions to be made on an unbounded scale, which can then be transformed back into probabilities using the logistic function:
$$ p = \frac{e^{Log \space Odds}}{1 + e^{Log \space Odds}} $$
This transformation ensures that predicted probabilities remain within the bounds of 0 and 1.  
\
\
For example lets assume you have a model predicting whether a student passes an exam based on hours studied. If the estimated log-odds for a student studying 5 hours is 1.5, you can calculate the probability as follows:  
First convert log-odds to odds:  
$$ Odds = e^{1.5} \approx 4.48 $$
Then Convert odds to probability:
$$ p = \frac{4.48}{1+4.48} \approx 0.817 . $$
\
\

### Logit function  
Using $1 - p = 1-(\frac{1}{1+exp(-x)}) = \exp(-x)/[1 + \exp(-x)]$ we can write the equation for the Log Odds function:  
$$ x = \ln(\frac{p}{1-p}) = \log(odds \space that \space Y=1 \space occurs) = logit\{Y=1\}$$  
...
with $X\beta$ equal to $\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \dots + \beta_k X_k$. Therefore the logistic model is a direct probability model since it is stated in terms of $Prob[Y=1 | X]$.
...
$$ \tt logit\{Y=1|X\} = \mit \beta_0 + \beta_1 X_1 + \dots + \beta_j X_j + \dots + \beta_k X_k$$
$$ = \beta_j X_j + C$$
...
$$ \tt Odds\{Y=1 | X\} = \mit \exp(B_j X_j + C) = \exp(\beta_j X_j) \exp(C) $$
...
## Example of Corollary Heart Disease  
...
- A small p-value for a predictor means adding that variable significantly improves model fit (it is associated with the outcome, adjusting for other model terms).  
- The overall table allows for sequential assessment of each term’s contribution as predictors are entered into the model in order.  
  
This helps identify which variables have meaningful effects and if any can be omitted from the model without significant loss of predictive power.
...
This estimate checks whether both coefficients are simultaneously equal to zero, that is, tests the null hypothesis: $H_0: \beta_{k5} = 0 \space \& \space \beta_{k618}=0$.  If the test result is significant (p << 0), it suggests that at least one of these variables contributes meaningfully to the model outcome. If the result is not significant, it implies that removing both predictors may not worsen the model fit significantly.  

```{r}
# To test if the coefficients are equal
linearHypothesis(mroz.mod, "k5 = k618")
```

This estimate checks whether both coefficients are simultaneously equal, it tests the null hypothesis $H_0 : \beta_{k5} = \beta_{k618}$.  If the test yields a significant result (low p-value), there is evidence that the coefficients are different from each other. If the result is not significant (high p-value), there is not enough evidence to say the two coefficients differ—the data are consistent with their effects being the same.
